  ...
  <div>
    ...
    <div class="right_box">
      <div class="listBox">
        <div class="right_item_box">
            ...
            <div class="mychart">
              ...
              <div class="lineBox">
                <div class="line">
                  <div
                    :style="{ width: personsInfo.local.percentage,background:personsInfo.local.color }"
                    class="line_left"
                  >
                  </div>
                  <div
                    :style="{ width: personsInfo.ontlocal.percentage,background:personsInfo.ontlocal.color }"
                    class="line_right"
                  >
                  </div>
                </div>
                <div class="textin">
                  <div>
                    <div class="number">{{ personsInfo.local.number }}</div>
                    <div class="text">{{ personsInfo.local.text }}</div>
                  </div>
                  <div>
                    <div class="number">{{ personsInfo.ontlocal.number }}</div>
                    <div class="text">{{ personsInfo.ontlocal.text }}</div>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div class="right_item_box">
            ...
            <div class="mychart">
              ...
              <div class="lineBox">
                <div class="line">
                  <div
                    :style="{ width: housesInfo.local.percentage,background:housesInfo.local.color }"
                    class="line_left"
                  >
                  </div>
                  <div
                    :style="{ width: housesInfo.ontlocal.percentage,background:housesInfo.ontlocal.color }"
                    class="line_right"
                  >
                  </div>
                </div>
                <div class="textin">
                  <div>
                    <div class="number">{{ housesInfo.local.number }}</div>
                    <div class="text">{{ housesInfo.local.text }}</div>
                  </div>
                  <div>
                    <div class="number">{{ housesInfo.ontlocal.number }}</div>
                    <div class="text">{{ housesInfo.ontlocal.text }}</div>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div class="right_item_box">
            <div class="new_title">车辆</div><div class="echarts">
            <div class="mychart">
              <!-- 柱图 -->
              <div class="lineBox">
                <div class="line">
                  <div
                    :style="{ width: carsInfo.local.percentage,background:carsInfo.local.color }"
                    class="line_left"
                  >
                  </div>
                  <div
                    :style="{ width: carsInfo.ontlocal.percentage,background:carsInfo.ontlocal.color }"
                    class="line_right"
                  >
                  </div>
                </div>
                <div class="textin">
                  <div>
                    <div class="number">{{ carsInfo.local.number }}</div>
                    <div class="text">{{ carsInfo.local.text }}</div>
                  </div>
                  <div>
                    <div class="number">{{ carsInfo.ontlocal.number }}</div>
                    <div class="text">{{ carsInfo.ontlocal.text }}</div>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div class="right_item_box">
            ...
            <div class="mychart">
              ...
              <div class="lineBox">
                <div class="line">
                  <div
                    :style="{ width: eventInfo.local.percentage,background:eventInfo.local.color }"
                    class="line_left"
                  >
                  </div>
                  <div
                    :style="{ width: eventInfo.ontlocal.percentage,background:eventInfo.ontlocal.color }"
                    class="line_right"
                  >
                  </div>
                </div>
                <div class="textin">
                  <div>
                    <div class="number">{{ eventInfo.local.number }}</div>
                    <div class="text">{{ eventInfo.local.text }}</div>
                  </div>
                  <div>
                    <div class="number">{{ eventInfo.ontlocal.number }}</div>
                    <div class="text">{{ eventInfo.ontlocal.text }}</div>
                  </div>
                </div>
              </div>
            </div>
      ...
      <div class="box_back bomBox">     
          <div class="bom_title">
              <div class="changebtn">
                  <div :class="{btn:1,active: rightbomIndex == index}" 
                  v-for="(item,index) in rightBomBtns"
                  @click="changeRightBomIndex(item,index)"
                    :key="index">{{item}}</div>
              </div>
              <div class="select">
                  <label>事件等级：</label>
                  <el-select v-model="eventValue" size="mini" @change="seleteChange" placeholder="请选择">
                    <el-option
          ...
          <div class="gropList">
              <vue-seamless-scroll
                ...
                >
                    ...
                    >
                    <div class="left">
                        <div class="spot" :style="{background: colors[`level${item['等级']}`]}"></div>
                        <img class="avatar" :src="item['图片']" alt="">
                        <div class="textInfo">
                            <div>{{item['名称']}}</div>
                            <div>{{item['备注']}}</div>
                            <div>{{item['时间']}}</div>
                    ...
                    <div class="right">
                        <div class="btn" @click.self="lookDetail(item)">查看</div>
                    </div>
                    </div>
                </vue-seamless-scroll>
          </div>
    ...
    <!-- 房屋详情 -->
    <template v-if="viewDetailsWsHouseShow">
        <ViewDetailsWsHouse @closeDetail='viewDetailsWsHouseShow = false' :personnelId="personnelIdws" />
    </template>
    <!-- 未实人员 -->
      <template v-if="undetailShow">
        <UnViewDetails :personnelId="personnelIdws"  @closeDetail="undetailShow = false" />
      </template>
      <!-- 未实车辆 -->
    <template v-if="undetailCarShow">
      <UsualCarLists :propLists="detailLists" @onClose="undetailCarShow = false" />
    </template>
    <!-- 楼层指挥平面图 -->
    <template v-if="commandPlanShow">
        <CommandPlan @closethis="commandPlanShow = false" :commname="community" :id="community == '香槟国际'?2:1" :items="items" />
    </template>
    <!-- 处理弹框 -->
    <div class="management" v-if="managementShow">
        <div class="showKuang">
            <MementBox :personnelId="personnelIdws"  @closeMent="managementShow = false" />
        </div>
    </div>
  </div>
</template>
<script>
  ...
  import ViewDetailsWsHouse from '../components/ViewDetailsWsHouse' // 房屋详情
  import UnViewDetails from '../components/UnViewDetails' // 未实人员详情
  import UsualCarLists from '../components/UsualCarLists' // 未实车辆详情
  ...
  export default {
    ...
    data () {
      return {
        // 未实车辆
        undetailCarShow: false,
        // 未实人员详情
        undetailShow: false,
        // 房屋详情
        viewDetailsWsHouseShow: false,
        // 重点人员处置
        ...
        total: 0,
        // 右侧底部按钮切换
        rightBomBtns:['未处理','已处理'],
        rightbomIndex: 0,// 选中
        // 事件等级
        eventValue: "",
        eventLevels:[
            {value: 1, label: 'Ⅰ级（特别严重）'},
            {value: 2, label: 'Ⅱ级（很严重）'},
            {value: 3, label: 'Ⅲ级（严重）'},
            {value: 4, label: 'Ⅳ级（一般严重）'},
        ],
        // 人数
        personsInfo: {
            local: {
    ...
    methods: {
        ...
        rightLine(){
            // 车辆
            this.$http.get(`${process.env.VUE_APP_5_URL}/home-right-car`).then(res =>{
                const nums = res.data['未实车辆'] + res.data['核实车辆']// 总数量
                let loc = '50%'
                let not = '50%'
                this.carsInfo.ontlocal.number = res.data['核实车辆']
                this.carsInfo.local.number = res.data['未实车辆']
                if(nums != 0){
                    loc =  (+res.data['未实车辆'] / +nums * 100) + '%' //计算左边的百分比
                    not =  (+res.data['核实车辆'] / +nums * 100) + '%' //计算左边的百分比
                }
                this.carsInfo.local.percentage = loc
                this.carsInfo.ontlocal.percentage = not
            })
            // 人数
            this.$http.get(`${process.env.VUE_APP_5_URL}/home-right-personnel`).then(res =>{
                const nums = res.data['未实人数'] + res.data['核实人数']// 总数量
                let loc = '50%'
                let not = '50%'
                this.personsInfo.ontlocal.number = res.data['核实人数']
                this.personsInfo.local.number = res.data['未实人数']
                if(nums != 0){
                    loc =  (+res.data['未实人数'] / +nums * 100) + '%' //计算左边的百分比
                    not =  (+res.data['核实人数'] / +nums * 100) + '%' //计算左边的百分比
                }
                this.personsInfo.local.percentage = loc
                this.personsInfo.ontlocal.percentage = not
            })
            // 房屋
            this.$http.get(`${process.env.VUE_APP_5_URL}/home-right-house`).then(res =>{
                const nums = res.data['空置房屋'] + res.data['排查空房']// 总数量
                let loc = '50%'
                let not = '50%'
                this.housesInfo.ontlocal.number = res.data['排查空房']
                this.housesInfo.local.number = res.data['空置房屋']
                if(nums != 0){
                    loc =  (+res.data['空置房屋'] / +nums * 100) + '%' //计算左边的百分比
                    not =  (+res.data['排查空房'] / +nums * 100) + '%' //计算左边的百分比
                }
                this.housesInfo.local.percentage = loc
                this.housesInfo.ontlocal.percentage = not
            })
            // 事件
            this.$http.get(`${process.env.VUE_APP_5_URL}/home-right-event`).then(res =>{
                const nums = res.data['发现事件'] + res.data['处置事件']// 总数量
                let loc = '50%'
                let not = '50%'
                this.eventInfo.ontlocal.number = res.data['处置事件']
                this.eventInfo.local.number = res.data['发现事件']
                if(nums != 0){
                    loc =  (+res.data['发现事件'] / +nums * 100) + '%' //计算左边的百分比
                    not =  (+res.data['处置事件'] / +nums * 100) + '%' //计算左边的百分比
                }
                this.eventInfo.local.percentage = loc
                this.eventInfo.ontlocal.percentage = not
            })
        },
        /**
         * 获取顶部导航数量
         ...
         */
        getRightGroupList(){
            this.$http.get(`${process.env.VUE_APP_5_URL}/home-right-bottom-list`,{params:{
                等级: this.eventValue,
                状态: this.rightbomIndex,
                current: 1,
                size: 100
            }}).then(res =>{
      ...
      lookDetail(item){
          ...
          const type = item['类型']
          this.personnelIdws = item['对象ID']
           //1.未实人，2.未实车辆，3.房屋空置，4.重点人群，5.消防报警
          switch(type){
              case 1:
                  this.undetailShow = true
              break;
              case 2:
                  this.detailLists.msg.ID = item['对象ID']
                  this.undetailCarShow = true
              break;
              case 3:
                  this.viewDetailsWsHouseShow = true
              break;
              case 4:
                  this.personnelIdws = item['对象ID'].toString()
                  this.managementShow = true
              break;
              case 5:
                  this.items = item['其他数据']
