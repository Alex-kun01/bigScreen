  ...
  <div>
    ...
    <div class="right_box">
      <div class="box_back echarts_box listBox">       
        ...
        <!-- <div class="bomPage">
            <span>共{{total}}条</span>
            <div class="page" @click="changepage('上一页')">上一页</div>
            <div class="page" @click="changepage('下一页')">下一页</div>
          </div> -->
      </div>
      ...
      getDengerList(){
        ...
        this.$http.post(`${process.env.VUE_APP_4_URL}/device-alarm/list`,{
          ...
          const newData = res.data.data.filter(item =>{
            ...
            if(localStorage.getItem('communityId') == 2){
              names = '青岛海信'
              communityName = '香槟国际'
            }
            if(localStorage.getItem('communityId') == 1){
              names = '海信工厂测试'
              communityName = '锦屏佳苑'
            }
            if(item.company_name == names){
              return true
            }
          })
